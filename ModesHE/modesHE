...
# Load data
csv = pd.read_csv('dispersionHE14.csv', sep=',')
dataframe = pd.DataFrame(csv)
dataframe.columns = ['Wavelength', 'Beta']
lamb = dataframe["Wavelength"]
beta = dataframe["Beta"]
lambda_ = lamb.iloc[1]  # primer valor
beta_HE11o = beta.iloc[1]
print(lambda_)
# Define functions
def n1(lambda_):
...
def Q_01(lambda_):
    return np.sqrt(beta_HE11o**2 - (k(lambda_))**2)
...
print(np.sum(density/integral) * dx * dy)
pd.DataFrame(density/np.sqrt(integral)).to_csv("mode_HE14.csv", index=False)
pd.DataFrame(field/np.sqrt(integral)).to_csv("mode_HE14_com.csv", index=False)
# Graficar
...
plt.title("$M_{14}f_{14}(x,y)\ \ HE_{14}$", fontsize=16)
plt.axis('equal')
plt.tight_layout()
plt.show()
